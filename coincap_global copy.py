...
if response.status_code == 200:
    ...
    try:
        ...
        last_updated_string = datetime.strptime(
            last_updated, '%Y-%m-%dT%H:%M:%S.%fZ').strftime('%B %d, %Y at %I:%M %p')
        print()
        print(f'There are currently {active_cryptocurrencies_string} active cryptocurrencies and {
              active_markets_string} active markets.')
        print(f'The global cap of all cryptos is {
              global_cap_string} USD and 24h global volume is {global_volume_string} USD.')
